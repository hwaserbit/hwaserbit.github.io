---
# 1. 필수 설정 (이건 꼭 수정하세요)
title: "포스트 제목 입력 (여기에 적으세요)"
date: 2025-12-24 16:00:00 +0900
categories: [Infrastructure, Server]
tags: [tag1, tag2, tag3]
description: "이 글의 핵심 요약을 여기에 적습니다.(구글 SEO 떄문)"
...
### 🧐 템플릿의 핵심 포인트 설명 (이건 읽어보세요)

1.  **Front Matter (맨 위 `---` 부분):**
    * **`pin: true`**: 이걸 켜면 글 날짜와 상관없이 블로그 **메인 화면 맨 위**에 압정처럼 고정됩니다. (공지사항이나 대표 프로젝트 글에 쓰세요.)
    * **`math`, `mermaid`**: 안 쓰는 글에서는 `false`로 두거나 지워야 페이지 로딩이 빠릅니다.

2.  **`{: .prompt-tip }` (알림 상자):**
    * `>` (인용) 바로 아랫줄에 저 코드를 붙이면 **예쁜 색깔 박스**로 변신합니다. 트러블 슈팅 글 쓸 때 **Error 로그는 Danger**, **해결책은 Tip**으로 쓰면 가독성이 끝내줍니다.

3.  **`{: file='파일명' }`:**
    * 코드 블럭(` ``` `) 바로 아랫줄에 이걸 붙이면, 코드 상단에 **파일명이 탭처럼** 붙습니다. (예: `_config.yml`) 엔지니어 블로그의 '간지'를 담당하는 기능입니다.
...
{% include bookmark.html 
  ...
  description="Learn how to install Docker Engine on Ubuntu using the repository." 
  image="https://docs.docker.com/assets/images/docs@2x.png" 
%}
```